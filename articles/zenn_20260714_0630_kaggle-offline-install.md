...
## 概要
Notebook提出のKaggleのコードコンペティションって、インターネット接続不可でも動くのが条件になんですが、プリインストールされていない外部ライブラリ(例: `zarr`)だと、プリインされてないがゆえにimport エラーになります。だからって、コードの先頭で`!pip install zarr`ってやっても提出版ではエラーになります。(`internet off`が条件だから)
